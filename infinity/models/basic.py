...
class SelfAttention(nn.Module):
    ...
    def extra_repr(self) -> str:
        tail = ''
        return f'using_flash={self.using_flash}, tau={self.tau}, cos_attn={self.cos_attn}{tail}, num_heads={self.num_heads}, head_dim={self.head_dim}'
...
class CrossAttention(nn.Module):
    ...
    def forward(self, q, ca_kv):
        ...
        oup = slow_attn(query=q_compact.view(B, Lq, self.num_heads, self.head_dim).permute(0,2,1,3),
                            key=kv_compact[:,0].view(B, max_seqlen_k, self.num_heads, self.head_dim).permute(0,2,1,3),
                            value=kv_compact[:,1].view(B, max_seqlen_k, self.num_heads, self.head_dim).permute(0,2,1,3),
                            scale=self.scale,
                            attn_mask=None,
                            dropout_p=0).permute(0,2,1,3).reshape(B, Lq, -1)
        return self.proj_drop(self.proj(oup))
