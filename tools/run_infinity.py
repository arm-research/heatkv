...
def load_infinity(
    ...
    with torch.cuda.amp.autocast(enabled=True, dtype=torch.bfloat16, cache_enabled=True), torch.no_grad():
        ...
        if checkpoint_type == 'torch':
            ...
            def load_hf_sharded_state_dict_into_model(model, model_path, strict=False, map_location="cpu"):
                index_file = None
                for candidate in [
                ...
                if index_file is None:
                    raise FileNotFoundError(
                        f"No sharded index file found in {model_path}. "
                ...
                for shard_name in shard_filenames:
                    ...
                    if shard_name.endswith(".safetensors"):
                        ...
                        shard_state = load_file(shard_path, device=map_location)
                    else:
                        shard_state = torch.load(shard_path, map_location=map_location)

                    full_state_dict.update(shard_state)

                missing_keys, unexpected_keys = model.load_state_dict(full_state_dict, strict=strict)
                return missing_keys, unexpected_keys


            missing, unexpected = load_hf_sharded_state_dict_into_model(
                infinity_test,
                model_path,
                strict=False,
            )
